...
app = FastAPI(title="Monitoring Agent", version="0.1.0", lifespan=lifespan)
...
async def execute_task(task: AgentTask) -> dict[str, Any]:
    agent: MonitoringAgent = app.state.agent
...
async def execute_task_stream(task: AgentTask) -> StreamingResponse:
    ...
    async def event_gen() -> AsyncGenerator[str, None]:
        sid, start = sse_session_start("monitoring", "agent_execute")
        yield start
        try:
            async for event in agent.execute(task):
...
async def chat_stream(req: ChatRequest) -> StreamingResponse:
    ...
    async def token_gen() -> AsyncGenerator[str, None]:
        sid, start = sse_session_start("monitoring", "agent_chat")
        yield start
