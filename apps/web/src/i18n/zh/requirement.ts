const requirement = {
  'requirement.create': '创建需求',
  'requirement.create.title': '标题',
  'requirement.create.description': '描述',
  'requirement.create.placeholder.title': '输入需求标题...',
  'requirement.create.placeholder.desc': '描述需求详情...',
  'requirement.empty': '暂无需求。新建一条后，由 AI 在工作流中持续推进实现。',
  'requirement.kanban.emptyColumn': '本阶段暂无',
  'requirement.listDesc':
    '在此记录目标与约束；由 AI 智能体编排工作流，拆解、执行并交付每一条需求。',
  'requirement.run': '运行分析',
  'requirement.reset': '重置并重新运行',
  'requirement.continue': '继续推进',
  'requirement.status.draft': '草稿',
  'requirement.status.designing': '设计中',
  'requirement.status.ready': '已就绪',
  'requirement.status.in_progress': '进行中',
  'requirement.status.completed': '已完成',
  'requirement.publish': '发布需求',
  'requirement.publishDesc': '需求设计完成，确认发布后开始执行',
  'requirement.publishConfirm': '确认发布',
  'requirement.unpublish': '撤回发布',
  'requirement.unpublishDesc': '撤回后退回设计阶段，可继续完善',
  'requirement.notReadyAlert': '需求尚未发布，请先完成设计并发布',
  'requirement.designModeHint': '与AI一起完善需求设计...',
  'requirement.executeModeHint': '@Agent 开始执行开发...',
  'requirement.phase.requirement': '需求分析',
  'requirement.phase.architecture': '架构设计',
  'requirement.phase.design': 'UI设计',
  'requirement.phase.development': '开发',
  'requirement.phase.testing': '测试',
  'requirement.phase.deployment': '部署',
  'requirement.phase.monitoring': '监控',
  'requirement.tasks': '任务',
  'requirement.artifacts': '制品',
  'requirement.relations': '关联需求',
  'requirement.relation.add': '添加关联',
  'requirement.relation.depends_on': '依赖',
  'requirement.relation.parent_of': '包含',
  'requirement.relation.related_to': '关联',
  'requirement.relation.evolves_from': '演进自',
  'requirement.relation.conflicts_with': '冲突于',
  'requirement.relation.depWarning': '依赖需求未完成',
  'requirement.relation.select': '选择需求...',
  'requirement.relation.type': '关联类型',
  'requirement.relation.empty': '暂无关联需求',
  'requirement.relation.noOther': '没有其他需求可以关联',
  'requirement.detail.back': '返回列表',
  'requirement.detail.expandDesc': '展开全文',
  'requirement.detail.collapseDesc': '收起',
  'requirement.detail.agentsSection': '智能体与工作区流程',
  'requirement.detail.tab.overview': '概览',
  'requirement.detail.tab.work': '阶段与任务',
  'requirement.detail.tab.agents': '智能体与流程',
  'requirement.detail.currentPhase': '当前阶段',
  'requirement.detail.goToWork': '查看阶段与任务',
  'requirement.detail.overviewHint': '在「阶段与任务」中选择阶段并管理任务列表。',
  'requirement.advance': '推进阶段',
  'requirement.scoped': '已筛选',
  'requirement.progress': '{done}/{total} 个任务',
  'requirement.noTasks': '此阶段暂无任务',
  'requirement.noArtifacts': '暂无制品',
  'requirement.deleteConfirmTitle': '删除需求',
  'requirement.deleteConfirmMsg': '删除「{title}」及其所有任务和制品？',
  'requirement.noneAssigned': '未分配',
  'requirement.aiSummary': 'AI 进展总结',
  'requirement.aiSummaryLoading': '正在生成总结…',
  'requirement.preview.label': '需求草稿',
  'requirement.preview.confirm': '确认创建',
  'requirement.preview.created': '需求已创建',
  'requirement.titlePlaceholder': '需求标题…',
  'requirement.descPlaceholder': '描述需求内容…',
}

export default requirement
